...
function verifyMfa(totp) {
  helpers.infoLog('Verifying MFA...')

    axios({
      url: 'https://api.paystack.co/verify_mfa',
      method: 'POST',
      data: { totp }
  }).then((response) => {
    ...
    helpers.errorLog(e.response.data.message || 'Unable to verify MFA, please try again in a few minutes');
...
function storeLoginDetails(payload) {
  ...
  db.write('token_expiry', payload.data.data.expiry)
  db.write('user', payload.data.data.user)
}

module.exports = {
    signIn, verifyMfa, getKeys, setWebhook, selectIntegration, getIntegration, pingWebhook
}
