    ...
    return new Promise((resolve, reject) => {
        ...
        axios({
            ...
            if (response && !response.mfa_required) {
              ...
              var [e, response] =  await helpers.promiseWrapper(verifyMfa(totp, response.token))
              if (response) {
                token = response.token
                user = response.user
                db.write('token', token)
...
module.exports = {
    signIn, verifyMfa, getKeys, setWebhook, selectIntegration, getIntegration, pingWebhook, refreshIntegration
}
