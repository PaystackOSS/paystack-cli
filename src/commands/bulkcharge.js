...
let key = 'bulkcharge';
let API = APIs[key];
class BulkchargeCommand extends Command {

  
  async run() {
    const {args, flags} = await this.parse(BulkchargeCommand)
    ...
    if(!schema){
      helpers.errorLog(`ValidationError: Invalid endpoint 'bulkcharge'`)
      return;
    }
...
BulkchargeCommand.description = helpers.getDescription(API, key)

BulkchargeCommand.flags = {
    domain: Flags.string(),
}
...
API.forEach(path => {
  ...
  path.params.forEach(param => {
    if (addedFlags.indexOf(param.parameter) < 0) {
      switch (param.type) {
      case 'String':
        BulkchargeCommand.flags[param.parameter] = Flags.string()
        break
      case 'Number':
        BulkchargeCommand.flags[param.parameter] = Flags.integer()
        break
      case 'Boolean':
        BulkchargeCommand.flags[param.parameter] = Flags.boolean()
        break
      default:
        BulkchargeCommand.flags[param.parameter] = Flags.string()
      }
  ...
  if(path.variables){
    path.variables.forEach((variable)=>{
      if(addedFlags.indexOf(variable.key) < 0){
        BulkchargeCommand.flags[variable.key] = Flags.string();
...
BulkchargeCommand.args = [
    {name: 'endpoint',   required: true, options: endpoints}
  ]


module.exports = BulkchargeCommand;
