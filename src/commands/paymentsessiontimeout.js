...
let key = 'paymentsessiontimeout';
let API = APIs[key];
class PaymentsessiontimeoutCommand extends Command {

  
  async run() {
    const {args, flags} = await this.parse(PaymentsessiontimeoutCommand)
    ...
    if(!schema){
      helpers.errorLog(`ValidationError: Invalid endpoint 'paymentsessiontimeout'`)
      return;
    }
...
PaymentsessiontimeoutCommand.description = helpers.getDescription(API, key)

PaymentsessiontimeoutCommand.flags = {
    domain: Flags.string(),
}
...
API.forEach(path => {
  ...
  path.params.forEach(param => {
    if (addedFlags.indexOf(param.parameter) < 0) {
      switch (param.type) {
      case 'String':
        PaymentsessiontimeoutCommand.flags[param.parameter] = Flags.string()
        break
      case 'Number':
        PaymentsessiontimeoutCommand.flags[param.parameter] = Flags.integer()
        break
      case 'Boolean':
        PaymentsessiontimeoutCommand.flags[param.parameter] = Flags.boolean()
        break
      default:
        PaymentsessiontimeoutCommand.flags[param.parameter] = Flags.string()
      }
  ...
  if(path.variables){
    path.variables.forEach((variable)=>{
      if(addedFlags.indexOf(variable.key) < 0){
        PaymentsessiontimeoutCommand.flags[variable.key] = Flags.string();
...
PaymentsessiontimeoutCommand.args = [
    {name: 'endpoint',   required: true, options: endpoints}
  ]


module.exports = PaymentsessiontimeoutCommand;
