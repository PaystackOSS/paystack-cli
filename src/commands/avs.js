...
let key = 'avs';
let API = APIs[key];
class AvsCommand extends Command {

  
  async run() {
    const {args, flags} = await this.parse(AvsCommand)
    ...
    if(!schema){
      helpers.errorLog(`ValidationError: Invalid endpoint 'avs'`)
      return;
    }
...
AvsCommand.description = helpers.getDescription(API, key)
...
API.forEach(path => {
  ...
  path.params.forEach(param => {
    if (addedFlags.indexOf(param.parameter) < 0) {
      switch (param.type) {
      case 'String':
        AvsCommand.flags[param.parameter] = Flags.string()
        break
      case 'Number':
        AvsCommand.flags[param.parameter] = Flags.integer()
        break
      case 'Boolean':
        AvsCommand.flags[param.parameter] = Flags.boolean()
        break
      default:
        AvsCommand.flags[param.parameter] = Flags.string()
      }
  ...
  if(path.variables){
    path.variables.forEach((variable)=>{
      if(addedFlags.indexOf(variable.key) < 0){
        AvsCommand.flags[variable.key] = Flags.string();
