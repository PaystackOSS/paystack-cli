...
class InvoiceCommand extends Command {
  ...
  async run() {
    const {args, flags} = await this.parse(InvoiceCommand)
    ...
    if(!schema){
      helpers.errorLog(`ValidationError: Invalid endpoint 'invoice'`)
      return;
    }
...
InvoiceCommand.description = helpers.getDescription(API, key)

InvoiceCommand.flags = {
    domain: Flags.string(),
}
...
API.forEach(path => {
  ...
  path.params.forEach(param => {
    if (addedFlags.indexOf(param.parameter) < 0) {
      switch (param.type) {
      case 'String':
        InvoiceCommand.flags[param.parameter] = Flags.string()
        break
      case 'Number':
        InvoiceCommand.flags[param.parameter] = Flags.integer()
        break
      case 'Boolean':
        InvoiceCommand.flags[param.parameter] = Flags.boolean()
        break
      default:
        InvoiceCommand.flags[param.parameter] = Flags.string()
      }
