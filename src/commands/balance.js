...
class BalanceCommand extends Command {
  ...
  async run() {
    const {args, flags} = await this.parse(BalanceCommand)
    ...
    if(!schema){
      helpers.errorLog(`ValidationError: Invalid endpoint 'balance'`)
      return;
    }
...
BalanceCommand.description = helpers.getDescription(API, key)
...
API.forEach(path => {
  ...
  path.params.forEach(param => {
    if (addedFlags.indexOf(param.parameter) < 0) {
      switch (param.type) {
      case 'String':
        BalanceCommand.flags[param.parameter] = Flags.string()
        break
      case 'Number':
        BalanceCommand.flags[param.parameter] = Flags.integer()
        break
      case 'Boolean':
        BalanceCommand.flags[param.parameter] = Flags.boolean()
        break
      default:
        BalanceCommand.flags[param.parameter] = Flags.string()
      }
  ...
  if(path.variables){
    path.variables.forEach((variable)=>{
      if(addedFlags.indexOf(variable.key) < 0){
        BalanceCommand.flags[variable.key] = Flags.string();
