...
class ProductCommand extends Command {
  ...
  async run() {
    const {args, flags} = await this.parse(ProductCommand)
    ...
    if(!schema){
      helpers.errorLog(`ValidationError: Invalid endpoint 'product'`)
      return;
    }
...
ProductCommand.description = helpers.getDescription(API, key)

ProductCommand.flags = {
    domain: Flags.string(),
}
...
API.forEach(path => {
  ...
  path.params.forEach(param => {
    if (addedFlags.indexOf(param.parameter) < 0) {
      switch (param.type) {
      case 'String':
        ProductCommand.flags[param.parameter] = Flags.string()
        break
      case 'Number':
        ProductCommand.flags[param.parameter] = Flags.integer()
        break
      case 'Boolean':
        ProductCommand.flags[param.parameter] = Flags.boolean()
        break
      default:
        ProductCommand.flags[param.parameter] = Flags.string()
      }
  ...
  if(path.variables){
    path.variables.forEach((variable)=>{
      if(addedFlags.indexOf(variable.key) < 0){
        ProductCommand.flags[variable.key] = Flags.string();
